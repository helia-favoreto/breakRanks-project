...
parser.add_argument('--glob', '-g', required=True,
                    help='glob pointing to *collisions.xml files')
...
#filePattern = re.compile("platooning_collisions.xml")
filePattern = re.compile("platooning_(\d+)_(\d+)_([-+]?(?:\d*\.*\d+))_(\d+)_collisions.xml")
...
if not files:
    print("No files matched the provided glob pattern.")
    exit()
...
for f in tqdm(files):
    ...
    if not collisions:
        ...
        print("no collision on this run")
        continue
    else:
